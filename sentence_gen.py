...
def sentence_gen(corpora, length=15):
    ...
    while len(words) < length:
        ...
        if len(words) + n >= length:
            filtered_ngrams = filter(lambda item: item[0][-1] is None, sorted_ngrams[n])
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', type=int, nargs='?', default=2, help="How many sentences?")
    parser.add_argument('-l', '--length', type=int, nargs='?', default=15, help="How long sentences?")
    args = parser.parse_args()
    
    for _ in range(args.n):
        print corpora.sentence_gen(length=args.length)
